...
using AutoGenTestTask.Service;
using AutoGenTestTask;

var workDir = Path.Combine(Path.GetTempPath(), "InteractiveService");
if (!Directory.Exists(workDir))
    Directory.CreateDirectory(workDir);

using var service = new MyCustomInteractiveService(workDir);
...
var gptConfig = LLMConfiguration.GetOpenAIGPT4();
...
var conversationHistory = await userProxy.InitiateChatAsync(groupChatManager, $"Сколько будет 1999 * 1000 {result}", maxRound: 10);
